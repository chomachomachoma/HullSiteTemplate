 ...
 */ (function ($) {
    $.fn.slides = function (options) {
        ...
        slides.addClass('slides-slide');

        // Phat Controller(s)
        if (settings.showControls && slideCount > 1) {

            // Create the elements for the controls
            var $controlContainer = $('<ul class="slides-controls"></ul>'),
                $next = $('<li><a href="#" class="slides-next" class="controls">' + settings.nextText + '</a></li>'),
                $previous = $('<li><a href="#" class="slides-prev" class="controls">' + settings.prevText + '</a></li>');

            // Bind click events to the controllers
            $next.click(function (e) {
                e.preventDefault();
                if (!animating) {
                    slidesGo(forward, false);
                }
            });
        ...
        if (settings.showMarkers && slideCount > 1) {
            ...
            var $markerContainer = $('<ol class="slides-markers"></ol>'),
        ...
        if (settings.useCaptions) {
            ...
            $.each(slides, function (key, value) {
                ...
                if (title) {
                    var $caption = $('<p class="slides-caption">' + title + '</p>');
        ...
        if (settings.animation === 'slide' && slideCount > 1) {
            ...
            $wrapper = $('<div class="slides-wrapper"></div>').css({
                'width': settings.width,
                'height': settings.height,
                'overflow': 'hidden',
                'position': 'relative'
        ...
        var slidesGo = function (direction, position) {
